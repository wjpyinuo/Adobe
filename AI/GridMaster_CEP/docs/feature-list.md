...
- `_clearGridMasterGuides` — 仅清除 GridMaster 创建的参考线
...
### 总清除
- `clearAll` — 一键清除所有 GridMaster 内容（参考线+覆盖层+构图线+印刷标记）
...
| 图层名 | 用途 |
|--------|------|
| `GridMaster_Overlays` | 半透明覆盖层 |
| `GridMaster_Composition` | 构图辅助线 |
| `GridMaster_PrintMarks` | 印刷标记 |
| `GridMaster_GridOverlay` | 网格色块覆盖 |
| `GridMaster_BaseGrid` | 基础参考线网格 |
| `GridMaster_Preview` | 实时预览线条 |
| `GridMaster_Ecom` | 电商安全区/标签/功能区 |
