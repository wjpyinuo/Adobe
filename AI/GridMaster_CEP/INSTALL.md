...
安装后重启 Illustrator → **窗口 → 扩展 → GridMaster**
...
将整个 `AI/GridMaster_CEP/` 文件夹复制到以下位置：

**Windows：**
```
%APPDATA%\Adobe\CEP\extensions\com.gridmaster.cep\
```

完整路径示例：
```
C:\Users\你的用户名\AppData\Roaming\Adobe\CEP\extensions\com.gridmaster.cep\
```

**macOS：**
```
~/Library/Application Support/Adobe/CEP/extensions/com.gridmaster.cep/
```

完整路径示例：
```
/Users/你的用户名/Library/Application Support/Adobe/CEP/extensions/com.gridmaster.cep/
```
...
ZXPSignCmd -sign AI/GridMaster_CEP gridmaster-cep.zxp cert.p12 yourPassword
```
...
2. 拖入 `gridmaster-cep.zxp`

**方式 B：使用 ExManCmd（Adobe 命令行工具）**
```bash
ExManCmd /install gridmaster-cep.zxp
```
...
```bash
xattr -r -d com.apple.quarantine ~/Library/Application\ Support/Adobe/CEP/extensions/com.gridmaster.cep/
```
