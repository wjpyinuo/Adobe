/**
 * GridMaster ExtendScript 宿主脚本 (完整版)
...
var _gridmasterUndoStack = [];
var _MAX_UNDO_STACK = 20;

function _pushUndoRecord(actionType, details) {
  _gridmasterUndoStack.push({
    type: actionType,
    details: details || {},
    timestamp: new Date().getTime()
  });
  if (_gridmasterUndoStack.length > _MAX_UNDO_STACK) {
    _gridmasterUndoStack.shift();
  }
}

function undoGridMaster() {
  try {
    if (_gridmasterUndoStack.length === 0) {
      return JSON.stringify({ success: false, error: 'No GridMaster actions to undo' });
    }

    var lastAction = _gridmasterUndoStack.pop();

    switch (lastAction.type) {
      case 'guides':
        _clearGridMasterGuides();
        break;
      case 'overlays':
    ...
    return JSON.stringify({
      ...
      remaining: _gridmasterUndoStack.length
    });
  } catch (e) {
...
function getUndoState() {
  return JSON.stringify({
    success: true,
    data: {
      count: _gridmasterUndoStack.length,
      lastAction: _gridmasterUndoStack.length > 0
        ? _gridmasterUndoStack[_gridmasterUndoStack.length - 1].type
        : null
    }
  });
}
...
// GridMaster 创建的参考线使用特殊命名前缀
var GM_GUIDE_PREFIX = 'GridMaster_';
...
/**
 * 仅清除 GridMaster 创建的参考线
 * 通过遍历带 GM_GUIDE_PREFIX 名称的 guide path 来识别
 */
function _clearGridMasterGuides() {
  try {
    var doc = app.activeDocument;
    // 遍历所有图层中的 guide 路径
...
return JSON.stringify({ success: true, message: 'All GridMaster content cleared' });
} catch (e) {
return JSON.stringify({ success: false, error: e.message });
}
}
...
var GRID_OVERLAY_LAYER_NAME = 'GridMaster_GridOverlay';
...
var BASEGRID_LAYER_NAME = 'GridMaster_BaseGrid';
...
var PREVIEW_LAYER_NAME = 'GridMaster_Preview';
...
var ECOM_LAYER_NAME = 'GridMaster_Ecom';
...
var abRight = abRect[2];
var abBottom = abRect[3];
var layer = getOrCreateLayer('GridMaster_PrintMarks');
clearLayerContents(layer);
var markLength = 10;
var markOffset = 5;
// 裁切标记（四角）
