 ...
 *
 * 加载顺序（index.html）：
 *   CSInterface.js → themeManager.js → lib/gridmaster-core.js →
...
(function () {
  ...
  var GM = window.GridMaster;
  if (!GM) { console.error('GridMaster core.js not loaded'); return; }
  ...
  function buildUI() {
    ...
    tabs.forEach(function (tab) {
      ...
      var isActive = tab.id === GM.currentTab;
      btn.style.cssText =
    ...
    // 根据当前 tab 渲染内容
    switch (GM.currentTab) {
      case 'grid': GM.renderGridPanel(content); break;
      case 'composition': GM.renderCompositionPanel(content); break;
      case 'ecom': GM.renderEcomPanel(content); break;
      case 'print': GM.renderPrintPanel(content); break;
      case 'ui': GM.renderUIPanel(content); break;
      case 'settings': GM.renderSettingsPanel(content); break;
    }
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'G') {
      e.preventDefault();
      if (GM.currentTab !== 'grid') { GM.currentTab = 'grid'; buildUI(); }
    ...
    if ((e.ctrlKey || e.metaKey) && e.key >= '1' && e.key <= '6') {
      ...
      if (tabIds[idx]) { GM.currentTab = tabIds[idx]; buildUI(); }
    }
  });
  ...
  // 定时刷新（兜底）
  setInterval(function () { GM.refreshDocInfo(); }, 5000);
